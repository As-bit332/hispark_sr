...
HI_VOID StVoParamCfg(VoCfg *self)
{
    ...
    self->enPicSize = g_lcdMediaInfo.enPicSize;
}

HI_VOID StVbParamCfg(VbCfg *self)
{
    memset_s(&g_lcdMediaInfo.vbCfg, sizeof(VB_CONFIG_S), 0, sizeof(VB_CONFIG_S));
     ...
     */
    g_lcdMediaInfo.u32BlkSize = COMMON_GetPicBufferSize(g_lcdMediaInfo.stSize.u32Width, g_lcdMediaInfo.stSize.u32Height,
        SAMPLE_PIXEL_FORMAT, DATA_BITWIDTH_8, COMPRESS_MODE_SEG, DEFAULT_ALIGN);
    self->astCommPool[0].u64BlkSize  = g_lcdMediaInfo.u32BlkSize;
     ...
     * Get raw buffer size
     */
    g_lcdMediaInfo.u32BlkSize = VI_GetRawBufferSize(g_lcdMediaInfo.stSize.u32Width, g_lcdMediaInfo.stSize.u32Height,
        PIXEL_FORMAT_RGB_BAYER_16BPP, COMPRESS_MODE_NONE, DEFAULT_ALIGN);
    self->astCommPool[1].u64BlkSize  = g_lcdMediaInfo.u32BlkSize;
...
{
    ViCfgInit(&g_lcdMediaInfo.viCfg);
    ViCfgSetDev(&g_lcdMediaInfo.viCfg, 0, -1);
    ViCfgSetPipe(&g_lcdMediaInfo.viCfg, 0, -1, -1, -1);
    g_lcdMediaInfo.viCfg.astViInfo[0].stPipeInfo.enMastPipeMode = 0;
    ViCfgSetChn(&g_lcdMediaInfo.viCfg, 0, -1, -1, -1);
    g_lcdMediaInfo.viCfg.astViInfo[0].stChnInfo.enCompressMode = 1;
}
...
int OrigImgToFrm(const IVE_IMAGE_S *img, VIDEO_FRAME_INFO_S* frm)
{
    ...
    printf("FRM before VirAdrr:0x%16.16llx, 0x%16.16llx, 0x%16.16llx\n", frm->stVFrame.u64VirAddr[0], frm->stVFrame.u64VirAddr[1], frm->stVFrame.u64VirAddr[2]);
    printf("FRM before PhyAdrr:0x%16.16llx, 0x%16.16llx, 0x%16.16llx\n", frm->stVFrame.u64PhyAddr[0], frm->stVFrame.u64PhyAddr[1], frm->stVFrame.u64PhyAddr[2]);
    ...
    frm->stVFrame.u64VirAddr[2] = frm->stVFrame.u64VirAddr[1];
    frm->stVFrame.u32Stride[2] = frm->stVFrame.u32Stride[1];
    printf("FRM Created VirAdrr:0x%16.16llx, 0x%16.16llx, 0x%16.16llx, %llu\n", frm->stVFrame.u64VirAddr[0], frm->stVFrame.u64VirAddr[1], frm->stVFrame.u64VirAddr[2], frm->stVFrame.u64VirAddr[1] - frm->stVFrame.u64VirAddr[0]);
    printf("FRM Created PhyAdrr:0x%16.16llx, 0x%16.16llx, 0x%16.16llx, %llu\n", frm->stVFrame.u64PhyAddr[0], frm->stVFrame.u64PhyAddr[1], frm->stVFrame.u64PhyAddr[2], frm->stVFrame.u64PhyAddr[1] - frm->stVFrame.u64PhyAddr[0]);
    return 0;
}
...
/*
 * 将sensor采集的图像显示在LCD屏幕上，即视频输入->视频处理子系统->视频输出->显示屏
 * Display the data collected by sensor to LCD screen, VI->VPSS->VO->MIPI
 */
int catchone = 0;
...
int SampleVioVpssVoMipi(void)
{
    ...
    HI_U32 ourWidth = 800, ourHeight = 480, ourChn = 2, ourStride = 800;
    HI_U32 inputWidth = 400, inputHeight = 240, inputChn = 3, FrmStride = 416;
    ...
    printf("!!!version 2023/8/18 13:23 \n");
    /*
     * 配置VI参数
     * Config VI parameter
     */
     ...
     */
    s32Ret = SAMPLE_COMM_VI_GetSizeBySensor(g_lcdMediaInfo.viCfg.astViInfo[0].stSnsInfo.enSnsType,
        &g_lcdMediaInfo.enPicSize);
    SAMPLE_CHECK_EXPR_RET(s32Ret != HI_SUCCESS, s32Ret, "get picture size by sensor failed, s32Ret=%#x\n", s32Ret);

    /*
     * 根据enPicSize，得到图片的宽高
     * Get picture size(w*h), according enPicSize
     */
    s32Ret = SAMPLE_COMM_SYS_GetPicSize(g_lcdMediaInfo.enPicSize, &g_lcdMediaInfo.stSize);
    ...
    printf("MediaInfo width = %d, height = %d\n", g_lcdMediaInfo.stSize.u32Width, g_lcdMediaInfo.stSize.u32Height);
    /*
     * 配置VB参数
     * Config VB parameter
     */
    StVbParamCfg(&g_lcdMediaInfo.vbCfg);

    /*
     * 视频缓存池初始化以及MPI系统初始化
     * VB init & MPI system init
     */
    s32Ret = SAMPLE_COMM_SYS_Init(&g_lcdMediaInfo.vbCfg);
    SAMPLE_CHECK_EXPR_RET(s32Ret != HI_SUCCESS, s32Ret, "system init FAIL, s32Ret=%#x\n", s32Ret);
     ...
     */
    VpssCfgInit(&g_lcdMediaInfo.vpssCfg);
    VpssCfgSetGrp(&g_lcdMediaInfo.vpssCfg, AIC_VPSS_GRP, NULL,
        g_lcdMediaInfo.stSize.u32Width, g_lcdMediaInfo.stSize.u32Width);
    g_lcdMediaInfo.vpssCfg.grpAttr.enPixelFormat = PIXEL_FORMAT_YVU_SEMIPLANAR_420;
    VpssCfgAddChn(&g_lcdMediaInfo.vpssCfg, AIC_VPSS_ZOUT_CHN, NULL, inputWidth, inputHeight);
    HI_ASSERT(!g_lcdMediaInfo.viSess);
    s32Ret = ViVpssCreate(&g_lcdMediaInfo.viSess, &g_lcdMediaInfo.viCfg, &g_lcdMediaInfo.vpssCfg);
    SAMPLE_CHECK_EXPR_GOTO(s32Ret != HI_SUCCESS, EXIT1, "ViVpss Sess create FAIL, ret=%#x\n", s32Ret);

    g_lcdMediaInfo.vpssGrp = AIC_VPSS_GRP;
    g_lcdMediaInfo.vpssChn0 = AIC_VPSS_ZOUT_CHN;

    /*
     * 配置VO参数
     * Config VO parameter
     */
    StVoParamCfg(&g_lcdMediaInfo.voCfg);

    /*
     * 启动VO到MIPI lcd通路
     * Start VO to MIPI lcd
     */
    s32Ret = SampleCommVoStartMipi(&g_lcdMediaInfo.voCfg);
    ...
    while (1) { 
        s32Ret = HI_MPI_VPSS_GetChnFrame(g_lcdMediaInfo.vpssGrp, g_lcdMediaInfo.vpssChn0, &stExtFrmInfo, s32MilliSec);
        if (s32Ret != HI_SUCCESS) {
            SAMPLE_PRT("Error(%#x),HI_MPI_VPSS_GetChnFrame failed, VPSS_GRP(%d), VPSS_CHN(%d)!\n", s32Ret, g_lcdMediaInfo.vpssGrp,
                as32VpssChn[1]);
            continue;
        }
        ...
        {   
            ...
            s32Ret = HI_MPI_VO_SendFrame(g_lcdMediaInfo.voCfg.VoDev, 0, &frm, s32MilliSec);
    ...
    EXT_RELEASE:
        s32Ret = HI_MPI_VPSS_ReleaseChnFrame(g_lcdMediaInfo.vpssGrp, g_lcdMediaInfo.vpssChn0, &stExtFrmInfo);
        if (s32Ret != HI_SUCCESS) {
            SAMPLE_PRT("Error(%#x),HI_MPI_VPSS_ReleaseChnFrame failed,Grp(%d) chn(%d)!\n", s32Ret, g_lcdMediaInfo.vpssGrp,
                g_lcdMediaInfo.vpssChn0);
        }
        bFirstFrm = HI_FALSE;
        HI_MPI_SYS_Munmap(ExtVirAddr, FrmStride * inputHeight * 3 / 2);
    }

    SAMPLE_SVP_NNIE_Ournet_Deinit(&s_stSegnetNnieParam,NULL,&s_stSegnetModel);
    SAMPLE_COMM_SVP_CheckSysExit();

    HI_MPI_SYS_Munmap(ourVirAddr, ourWidth * ourHeight * 3 / 2);
    HI_MPI_VB_ReleaseBlock(ourblock);
    Pause();

    SAMPLE_COMM_VPSS_UnBind_VO(g_lcdMediaInfo.vpssGrp, g_lcdMediaInfo.vpssChn0, g_lcdMediaInfo.voCfg.VoDev, 0);
    SAMPLE_VO_DISABLE_MIPITx(fd);
    SampleCloseMipiTxFd(fd);
    system("echo 0 > /sys/class/gpio/gpio55/value");

EXIT2:
    SAMPLE_COMM_VO_StopVO(&g_lcdMediaInfo.voCfg);
EXIT1:
    VpssStop(&g_lcdMediaInfo.vpssCfg);
    SAMPLE_COMM_VI_UnBind_VPSS(g_lcdMediaInfo.viCfg.astViInfo[0].stPipeInfo.aPipe[0],
        g_lcdMediaInfo.viCfg.astViInfo[0].stChnInfo.ViChn, g_lcdMediaInfo.vpssGrp);
    ViStop(&g_lcdMediaInfo.viCfg);
EXIT:
