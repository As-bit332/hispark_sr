...
def calculate_lpips(img, img2, crop_border, input_order='HWC', test_y_channel=False, **kwargs):
    ...
    img = img.astype(np.float64)
    img2 = img2.astype(np.float64)
    res = lp.im2tensor(img)
    ref = lp.im2tensor(img2)
    lpips_fn = lp.LPIPS(net="alex")
    score = lpips_fn(res, ref)
    return score.item()


@METRIC_REGISTRY.register()
def calculate_dists(img, img2, crop_border, input_order='HWC', **kwargs):
    assert img.shape == img2.shape, (f'Image shapes are different: {img.shape}, {img2.shape}.')
    if input_order not in ['HWC', 'CHW']:
        raise ValueError(f'Wrong input_order {input_order}. Supported input_orders are "HWC" and "CHW"')
    img = reorder_image(img, input_order=input_order)
    ...
    img2 = torch.from_numpy(img2) / 255.0
    img2 = img2.to(torch.float32)
    img = torch.unsqueeze(img, 0).permute(0, 3, 1, 2)
    img2 = torch.unsqueeze(img2, 0).permute(0, 3, 1, 2)

    D = DISTS()
    dists_value = D(img, img2)
