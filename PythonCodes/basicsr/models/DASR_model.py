    ...
    def feed_data(self, data_all):
        if self.is_train:
            ...
            if self.degradation_degree == 'severe_degrade_two_stage':
                ...
                self.kernel1 = data['kernel1']['kernel'].to(self.device)  # 因为DASR_dataset中data['kernel1']返回的是kernel_info这个列表，列表中的kernel元素才是self.kernel1
                ...
                if np.random.uniform() < self.opt['gaussian_noise_prob']:
                    ...
                    = random_add_gaussian_noise_pt(out, sigma_range=self.opt['noise_range'], clip=True, rounds=False, gray_prob=gray_noise_prob)
                    ...
                    scale, gray_noise, out, self.noise_p_first \
                        = random_add_poisson_noise_pt(out, scale_range=self.opt['poisson_scale_range'], gray_prob=gray_noise_prob, clip=True, rounds=False)
