...
def channel_shuffle(x, groups):
    ...
    # reshape
    x = x.view(b, n, groups, channel_per_group)
...
class MultiScaleDWConv(nn.Module):
    def __init__(self, dim, scale=(1, 3, 5, 7)):
        ...
        self.act = nn.GELU()
        self.alpha = nn.Parameter(torch.ones(dim, 1, 1), requires_grad=True)
        self.bata  = nn.Parameter(torch.ones(dim, 1, 1), requires_grad=True)
    ...
    def forward(self, x):
        ...
        max_pool = F.max_pool2d(x, kernel)  # (B,C,1,1)
        pool_sum = self.alpha * avg_pool + self.bata * max_pool

        pool_sum = self.bottleneck(pool_sum)

        sig_pool = torch.sigmoid(pool_sum)  # (B,C,1,1)
        # sig_pool = sig_pool.unsqueeze(2).unsqueeze(3)  # (B,C,1,1)
        sig_pool = sig_pool.repeat(1, 1, kernel[0], kernel[1])  # (B,C,H,W)
...
class CCaFFN(nn.Module):
    def __init__(self,
        ...
        self.fc1 = nn.Sequential(
            # nn.Conv2d(in_features, hidden_features, kernel_size=1, bias=True),
    ...
    def forward(self, x, x_size):
        self.scale = nn.Parameter(torch.ones(x.shape[2] * 2, x_size[0], x_size[1]), requires_grad=True).cuda()
        x = self.fc1(x)

        x = x.view(x.shape[0], x.shape[2], x_size[0], x_size[1]).contiguous()
        x = self.dwconv(x) + self.scale * x
...
class PSA(nn.Module):  # Spatial-wise SA
    ...
    def forward(self, x, mask=None):
        """
        Args:
            x: input features with shape of (num_windows*b, n, c) n = ws * ws
            mask: (0/-inf) mask with shape of (num_windows, Wh*Ww, Wh*Ww) or None
        """
        b_, n, c = x.shape
        kv = self.kv(x)
        # compress the channel dimension of K,V :(num_windows*b, num_heads, n//4, c//num_heads)
        # 后面一组reshape+permute是分头操作
...
class PSA_Block(nn.Module):
    ...
    def __init__(self,
        ...
        self.norm1 = norm_layer(num_sdim)
        self.norm2 = norm_layer(num_sdim)
        ...
        mlp_hidden_dim = int(self.num_sdim * mlp_ratio)
        self.mlp = CCaFFN(in_features=self.num_sdim, hidden_features=mlp_hidden_dim, drop=drop)
    ...
    def forward(self, x, x_size):
        ...
        if self.num_sdim > c // 2:
            x_s = torch.split(x, self.num_sdim, dim=-1)
            x, x_shuffle = x_s[0], x_s[1]  # x[b, n, num_sdim] x_shuffle[b, n, dim-num_sdim]
        else:
            x_s = torch.split(x, c - self.num_sdim, dim=-1)
            x, x_shuffle = x_s[1], x_s[0]

        shortcut = x
        x = self.norm1(x)
        x = self.after_norm1(x)
        x = x.view(b, h, w, self.num_sdim)
        ...
        x_windows = x_windows.view(-1, self.window_size * self.window_size, self.num_sdim)  # nw*b, window_size*window_size, c
        ...
        # merge windows
        attn_windows = attn_windows.view(-1, self.window_size, self.window_size, self.num_sdim)
    ...
    def __init__(self,
        ...
        self.blocks = nn.ModuleList([
            PSA_Block(
                ...
                num_sdim=num_sdim) for i in range(depth)
        ])
